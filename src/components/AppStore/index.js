...
class AppStore extends Component {
  state = {activeTabId: tabsList[0].tabId, inputString: ''}
  ...
  render() {
    const {activeTabId, inputString} = this.state
    const filteredList = this.getFilteredList()
    const filterInputList = filteredList.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(inputString.toLowerCase()),
    )
    return (
